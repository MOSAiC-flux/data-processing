...
#
# The plots are done in parallel, so your lap could get a little hot...
#
...
def main(): # the main data crunching program
    ...
    try:
        ...
        for i_day, start_day in enumerate(day_series): # loop over the days in the processing range and get a list of files
            if i_day % nthreads!=0 or i_day>len(day_series): continue
            df_today = pd.DataFrame()
            q_dict = {}
            p_dict = {}
            df_dict = {}
            
            print("  ... getting data for day {} (and {} days after in parallel)".format(start_day,nthreads))
            for ithread in range(0,nthreads): 
                today = start_day + timedelta(ithread)
                file_str_slow = '/slow_preliminary_tower.{}.nc'.format(today.strftime('%Y%m%d'))

                level1_dir    = data_dir+'/tower/1_level_ingest/'
                curr_file     = level1_dir+file_str_slow

                q_dict[ithread] = Q()
                #if today > day_series[-1]: q_dict[ithread].put(pd.DataFrame()); continue
                p_dict[ithread] = P(target=get_tower_data, \
                                    args=(curr_file, ithread, today, \
                                          q_dict[ithread])).start()


            for ithread in range(0,nthreads):
                df_dict[ithread] = q_dict[ithread].get()
                cv = q_dict[ithread].get()
                if cv!=None: code_version = cv

            for ithread in range(0,nthreads):
                if df_today.empty: df_today = df_dict[ithread]
                else: df_today = pd.concat( [df_today, df_dict[ithread]],axis=0)

            df_list.append(df_today.copy())

        print("\n ... concatting DFs, might take a second... ")
        df = pd.concat(df_list)
        df = df.sort_index()
        time_dates = df.index
        df['time'] = time_dates # duplicates index... but it can be convenient

        print(" ... done, here's a data sample :")
        print('================')
        print(df)
        print('\n')
        print(df.info())
        ...
        print("An exception in main.... returning")
        print("-----------")
        import traceback
        traceback.print_exc()
        print("=============================================")
...
def make_plot(df, subplot_dict, units, colors, save_str, daily, q):
    ...
    for subplot_name, var_list in subplot_dict.items():
        ...
        for var in var_list:
            ...
            if ('vaisala_T' or 'vaisala_RH') in var:
...
# drop me to an interactive prompt when I request it so I can have fun
def drop_me(locs=None,exit=False):
    import code, readline, rlcompleter                                           

    try: all_vars = {**globals(), **locs}
    except: all_vars = {**globals(), **locals()} 
    if prompt_yn('Do you want to drop to a prompt here?'):
        code.InteractiveConsole(all_vars).interact()
    if exit==True: exit()
    readline.set_completer(rlcompleter.Completer(all_vars).complete) 
    readline.parse_and_bind("tab: complete")                     

def prompt_yn(prompt_str,default=True):
    import click # let a library do the hard work
    click.confirm(prompt_str)
